return {
  {
    ...
    init = function()
      ...
      keys[#keys + 1] = {"<leader>tD", function() require("neotest").run.run({ strategy = "dap" }) end,
        desc = "Debug test file", mode = "n" }
      -- debug nearest test
      --keys[#keys + 1] = {"<leader>td", function() require("neotest").run.run({ vim.fn.expand("%"), strategy = "dap" }) end,  mode = "n" }
      keys[#keys + 1] = {"<leader>td", function() require("neotest").run.run({ vim.fn.expand("%"), strategy = "dap" }) end,
        desc = "Debug nearest test", mode = "n" }
    end,
  },
  {
